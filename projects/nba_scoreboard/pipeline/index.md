...
To support real‑time game ratings and matchup insights, the app pulls together data from a number of sources. My first version of this project scraped scoreboard off the web, but I’ve migrated over to the [NBA API package](https://github.com/swar/nba_api) — a community‑maintained Python wrapper around the NBA’s murky internal API. It’s an amazing resource for fans: well‑structured, actively supported, and easier/more reliable than scraping HTML. The pipeline pulls from ten different endpoints spanning live box scores, advanced stats, betting odds, and player‑tracking data (play types, shot locations, and movement‑based metrics).
Basketball Reference is also an important complementary source. It’s extremely rich, very scrapable, and has some data that the NBA API doesn’t yet provide. I still rely on it for the daily injury report, and since I had already built a bridge between Basketball Reference and the NBA API, it made sense to consolidate more player‑level data on advanced metrics and awards there as well.
