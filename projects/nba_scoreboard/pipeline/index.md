...
The pipeline is designed to be a considerate data consumer. I pull everything up front, recycle as much as possible, and cache aggressively given Streamlit’s auto-refresh architecture. Some NBA API endpoints can be slow, and the league does throttle requests, so avoiding unnecessary repulls is important for both performance and staying within rate limits. A two-tier caching strategy ensures that slow-changing data elements are only pulled daily, whereas live elements autorefresh every five minutes during live play. I’ve added exception handling around the more fragile components (especially the odds API), to ensure the app remains stable even when upstream data is spotty.
